...
def login_client_and_user(client_id, client_secret, username, password, url):
    headers = {'Content-Type': 'application/x-www-form-urlencoded',
               'Accept': 'application/json'}
    payload = {'grant_type': 'client_credentials',
               'response_type': 'token',
               'token_format': 'opaque'}
    response = requests.post(f'http://{url}/oauth/token?username={username}&password={password}&token_format=opaque&response_type=token', headers=headers,
                             params=payload,
...
token = login_client_and_user(client_id=args.client_id, client_secret=args.client_secret, username=args.username,
                              password=args.password, url=args.url)

query_user(access_token=token, username=args.username, url=args.url)
